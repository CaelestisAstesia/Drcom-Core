...
## 📢 项目维护公告：进入 LTS 阶段 (LTS Announcement)

鉴于 **Drcom-Core v1.1.0** 的异步重构已达成预期的现代化与稳定性目标，我们将此版本正式标记为 **长期支持版本 (Long-Term Support)**。

* **API 冻结 (API Freeze)**：即日起冻结核心引擎与策略接口规范，不再进行破坏性变更，以确保对上层应用（CLI/GUI/Service）提供持久且稳定的向后兼容性。
* **发布策略调整**：除非有新的开发者贡献针对其他院校/设备的协议适配 (Protocol Strategy) 或提交重大特性，否则本项目将**停止发布新的功能版本**。未来的维护工作将严格限制在 Bug 修复与安全性更新。
* **社区贡献呼吁**：得益于 v1.1.0 的“引擎-策略”解耦架构，适配新协议已变得非常简便。我们诚挚欢迎各校开发者提交 Pull Request 以扩展支持范围；只有在收到此类实质性贡献时，我们才会重启功能版本的发布计划。
