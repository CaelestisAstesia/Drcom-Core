...
def build_keep_alive2_packet_custom_tail_logic(
    ...
    if not isinstance(tail_to_use, bytes) or len(tail_to_use) != 4:
        print(f"错误：tail_to_use 无效或长度不为 4 字节。")
        return None
    ...
    if not isinstance(keep_alive_version, bytes) or len(keep_alive_version) != 2:
        print(f"错误：keep_alive_version 无效。")
        return None
...
def build_keep_alive2_packet_py3(
    ...
    if packet_type not in [1, 3]:
        ...
        return None
    if not isinstance(keep_alive_version, bytes) or len(keep_alive_version) != 2:
        print(f"错误：keep_alive_version 无效或长度不为 2 字节。")
        return None
...
if __name__ == '__main__':
    ...
    packet1 = build_keep_alive2_packet_py3(
        current_packet_number, current_tail, 1,
        example_host_ip, example_keep_alive_version, is_first_packet=True
    )
    if packet1:
        print(f"\n构建: 第 {current_packet_number} 包 (Type 1, First)")
        print(f"  Hex: {binascii.hexlify(packet1).decode()}")
        # 模拟收到响应1
        simulated_response1_data = b'\x07\x00\x28\x00\x0b\x02' + example_keep_alive_version + \
            b'\x2f\x12' + b'\x00'*6 + b'\xaa\xbb\xcc\xdd' + b'\x00'*20  # 假设响应
    ...
    packet2 = build_keep_alive2_packet_py3(
        current_packet_number, b'\x00'*4, 1,  # 仍然用 0000 发送
        example_host_ip, example_keep_alive_version, is_first_packet=False
    )
    if packet2:
        print(f"\n构建: 第 {current_packet_number} 包 (Type 1)")
        print(f"  Hex: {binascii.hexlify(packet2).decode()}")
        # 模拟收到响应2
        simulated_response2_data = b'\x07\x01\x28\x00\x0b\x02' + example_keep_alive_version + \
            b'\x2f\x12' + b'\x00'*6 + b'\x11\x22\x33\x44' + b'\x00'*20
    ...
    packet3 = build_keep_alive2_packet_py3(
        current_packet_number, current_tail, 3,  # 使用响应2的 tail
        example_host_ip, example_keep_alive_version, is_first_packet=False
    )
    if packet3:
        ...
        # 模拟收到响应3
        simulated_response3_data = b'\x07\x02\x28\x00\x0b\x04' + example_keep_alive_version + \
            b'\x2f\x12' + b'\x00'*6 + b'\x55\x66\x77\x88' + b'\x00'*20
    ...
    packet4 = build_keep_alive2_packet_py3(
        current_packet_number, current_tail, 1,  # 使用响应3的 tail
        example_host_ip, example_keep_alive_version, is_first_packet=False
    )
    if packet4:
        ...
        # 模拟收到响应4
        simulated_response4_data = b'\x07\x03\x28\x00\x0b\x02' + example_keep_alive_version + \
            b'\x2f\x12' + b'\x00'*6 + b'\xab\xcd\xef\x01' + b'\x00'*20
    ...
    packet5 = build_keep_alive2_packet_py3(
        current_packet_number, current_tail, 3,  # 使用响应4的 tail
        example_host_ip, example_keep_alive_version, is_first_packet=False
    )
    if packet5:
        ...
        # 模拟收到响应5
        simulated_response5_data = b'\x07\x04\x28\x00\x0b\x04' + example_keep_alive_version + \
            b'\x2f\x12' + b'\x00'*6 + b'\xfe\xdc\xba\x98' + b'\x00'*20
