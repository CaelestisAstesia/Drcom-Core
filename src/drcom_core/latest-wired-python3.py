...
# Process MAC address: remove separators and convert to integer
mac_str = os.getenv("MAC", "").replace("-", "").replace(":", "")
mac = int(mac_str, 16) if mac_str else 0

# Convert hex strings from .env to bytes
CONTROLCHECKSTATUS = bytes.fromhex(os.getenv("CONTROLCHECKSTATUS", "20"))
ADAPTERNUM = bytes.fromhex(os.getenv("ADAPTERNUM", "01"))
IPDOG = bytes.fromhex(os.getenv("IPDOG", "01"))
AUTH_VERSION = bytes.fromhex(os.getenv("AUTH_VERSION", "0a00"))
KEEP_ALIVE_VERSION = bytes.fromhex(os.getenv("KEEP_ALIVE_VERSION", "dc02"))

# String configurations
host_name = os.getenv("HOST_NAME", "JLU_CLIENT")
host_os = os.getenv("HOST_OS", "Windows 10")
PRIMARY_DNS = os.getenv("PRIMARY_DNS", "10.100.61.1")
dhcp_server = os.getenv("DHCP_SERVER", "0.0.0.0")
...
def ror(md5, pwd):
    ret = bytearray()  # Use a bytearray to build the result
...
def keep_alive2(*args):
    ...
    while True:
        try:
            ...
            s.sendto(packet, (svr, 61440))
            log(
                "[keep_alive2] send", str(i + 1), str(binascii.hexlify(packet))[2:][:-1]
            )
            data, address = s.recvfrom(1024)
            log("[keep_alive2] recv", str(binascii.hexlify(data))[2:][:-1])
            tail = data[16:20]
...
def mkpkt(salt, usr, pwd, mac):
    ...
    data += b"".join(
        [bytes([int(i)]) for i in PRIMARY_DNS.split(".")]
    )  # _tagHostInfo.DNSIP1
    data += b"".join(
        [bytes([int(i)]) for i in dhcp_server.split(".")]
    ...
    data += checksum(
        data + b"\x01\x26\x07\x11\x00\x00" + dump(mac)
