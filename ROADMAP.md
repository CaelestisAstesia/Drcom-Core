...
* [ ] **P 版协议支持**：分析 `drcom-generic` 中的 P 版（PPPoE）逻辑，并将其封装为 `drcom_protocol` 下的一个新模块。~~真的有地方还在拨号上网吗~~
* [ ] **打包与发布**：将项目配置 `pyproject.toml` 打包。
* [ ] **配置收集**：建立一个机制来收集和展示来自不同学校的 `.env` 配置模板。

## 长期目标 (Long-Term / "项目集")

* [ ] **GUI 客户端**：利用 `drcom-core` API，开发一个独立的、跨平台的图形界面客户端（例如使用 PyQt/PySide）。
* [ ] **Web 管理面板**：开发一个轻量级的 Web 服务（例如使用 FastAPI 或 Flask），用于在路由器或树莓派上远程管理 `drcom-core`。
* [ ] **兼容 `drcom-generic`**：最终目标是能够解析和兼容 `drcom-generic` `custom/` 目录 下的大多数 D 版和 P 版脚本配置。
